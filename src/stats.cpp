...
// lime
//
// Copyright © 2020-2025 elmerucr. All rights reserved.
// ---------------------------------------------------------------------
...
void stats_t::process_parameters()
{
	...
	if (framecounter == framecounter_interval) {
		 ...
		 * cpu speed
		 */
		cpu_ticks = system->core->cpu->getClock();
	...
	if (status_bar_framecounter == status_bar_framecounter_interval) {
		...
		snprintf(statistics_string, 256,
			"\n frametime:%7.2f ms      host cpu:%6.1f %%\n"
			"      core:%7.2f ms   audiobuffer: %5.1f ms\n"
			"       cpu:%7.2f MHz    framerate:%6.1f fps",
