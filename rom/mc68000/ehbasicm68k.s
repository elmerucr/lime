...
*	example code for the EASy68k editor/simulator.		*
*	2002/3/4/5.						*
*								*
*****************************************************************
*								*
*	Copyright (C) 2002/3/4/5 by Lee Davison. This program	*
...
VEC_OUT
	...
	moveq	#1,D0		* character out (function one in trap #15)
...
*##	move.b	#$F2,Iclim(a3)		* default limit for TAB = 14
...
LAB_13A6
	...
	move.b	d1,Oquote(a3)		* clear open quote/DATA flag
LAB_13AC
...
LAB_1477
	...
	move.b	-(a5),d0			* -1 (as end of previous line)
...
*	BSR.s		LAB_1212			* check room on stack for d0 bytes
	...
	move.l	a0,(sp)			* push onto stack (and dump return address)
	...
	move.b	Dtypef(a3),-(sp)		* push FOR variable data type onto stack
...
LAB_1754
	BSR		LAB_IGBY			* increment & scan memory, Cb=1 if "0"-"9"
...
LAB_IF
	...
	BNE		LAB_SNER			* if not "THEN" token do syntax error/warm start
...
LAB_READ
	MOVEA.l	Dptrl(a3),a0		* get DATA pointer
...
LAB_195B
	...
	BMI.s		LAB_19DD			* branch if READ (go find next statement)
							...
							* mode was INPUT
	BSR		LAB_18E3			* print "?" character, double ? for extended
							* input
	BSR		LAB_INLN			* print "? " and get BASIC input
							* return a0 pointing to the buffer start
	TST.b		(a0)				* test first byte from buffer
...
LAB_19B6
	...
	ADDQ.w	#1,a5				* else was "," so point to next chr
							* got good input data
LAB_19C2
	move.l	a5,Rdptrl(a3)		* save read pointer for now
	MOVEA.l	(sp)+,a5			* restore execute pointer
	BSR		LAB_GBYT			* scan memory
...
LAB_1A46
	...
	moveq		#$1A,d1			* set for FOR use size
...
LAB_EVEX
	move.b	-(a5),d1			* decrement BASIC execute pointer (faster)
LAB_EVEZ
...
LAB_BHSS
	...
	move.b	(a5)+,d2			* get BASIC byte
	CMP.b		#',',d2			* compare with comma
	BNE		LAB_BHCB			* if not "," go check close bracket
...
LAB_1D12
	...
	move.b	d1,Dtypef(a3)		* clear data type, $80=string, $40=integer,
							* $00=float
...
LAB_1DD7
	move.b	#$00,Dtypef(a3)		* clear data type
	BTST.l	#23,d0			* was it string
	BEQ.s		LAB_1DD8			* branch if not string

	move.b	#$80,Dtypef(a3)		* set data type = string
...
LAB_1DD8
	...
	move.b	#$40,Dtypef(a3)		* set data type = integer
LAB_1DD9
...
LAB_DEFV
	...
	MOVEA.l	func_l(a3),a0		* get pointer
...
LAB_224D
	...
	MOVEA.l	FAC2_m(a3),a0		* retreive result string pointer
	move.l	Sutill(a3),d1		* copy end
...
LAB_LCASE
	...
	BEQ.s		NoString			* branch if null string
							...
							* else copy and change string
	move.l	d0,d1				* copy string length
	MOVEA.l	a0,a1				* copy string address
	BSR		LAB_2115			* make string space d1 bytes long
	ADDA.l	d1,a0				* new string end
	ADDA.l	d1,a1				* old string end
...
LAB_SWAP
	BSR		LAB_GVAR			* get variable address in a0
	MOVEA.l	a0,a2				* copy address
	move.b	Dtypef(a3),d3		* get data type, $80=string, $40=inetger,
							* $00=float

	BSR		LAB_1C01			* scan for "," , else do syntax error/warm start
	BSR		LAB_GVAR			* get var2 address
							* return pointer to variable in Cvaral and a0
	CMP.b		Dtypef(a3),d3		* compare with var2 data type
	BNE		LAB_TMER			* if not both the same type do "Type mismatch"
							* error then warm start

	move.l	(a0),d0			* get var2
	move.l	(a2),(a0)+			* copy var1 to var2
	move.l	d0,(a2)+			* save var2 to var1
...
LAB_284G
	...
	BCS		LAB_284L			* if < minimum just do shift
...
LAB_2989
	...
	move.l	#$F4240000,FAC2_m(a3)	* 1000000 mantissa
	MOVE.w	#$9400,FAC2_e(a3)		* 1000000 exponent & sign
	move.b	FAC1_s(a3),FAC_sc(a3)	* make FAC1 sign sign compare
	BSR		LAB_MULTIPLY		* do FAC2*FAC1

	move.b	#$FA,numexp(a3)		* set number exponent count (-6)
LAB_299C
	move.l	#$F423F700,FAC2_m(a3)	* set mantissa for 999999.4375
	MOVE.w	#$9400,FAC2_e(a3)		* set exponent for 999999.4375
	...
	BHI.s		LAB_29B9			* go do /10 if FAC1 > 999999.4375

	BEQ		LAB_29C3			* branch if FAC1 = 999999.4375

							* FAC1 < 999999.4375
	move.l	#$C34FF800,FAC2_m(a3)	* set mantissa for 99999.9375
	MOVE.w	#$9100,FAC2_e(a3)		* set exponent for 99999.9375
...
LAB_2Y04
	...
	move.b	1(a0,d4.w),FAC2_e(a3)	* copy exponent for multiply
...
TK_THEN		EQU TK_SPC+1		* $AC
TK_NOT		EQU TK_THEN+1		* $AD
...
TK_GT			EQU TK_LSHIFT+1		* $BB
TK_EQUAL		EQU TK_GT+1			* $BC
TK_LT			EQU TK_EQUAL+1		* $BD
TK_SGN		EQU TK_LT+1			* $BE
...
TK_LEN		EQU TK_LEEK+1		* $CF
TK_STRS		EQU TK_LEN+1		* $D0
TK_VAL		EQU TK_STRS+1		* $D1
TK_ASC		EQU TK_VAL+1		* $D2
TK_UCASES		EQU TK_ASC+1		* $D3
TK_LCASES		EQU TK_UCASES+1		* $D4
TK_CHRS		EQU TK_LCASES+1		* $D5
TK_HEXS		EQU TK_CHRS+1		* $D6
TK_BINS		EQU TK_HEXS+1		* $D7
TK_BITTST		EQU TK_BINS+1		* $D8
TK_MAX		EQU TK_BITTST+1		* $D9
TK_MIN		EQU TK_MAX+1		* $DA
TK_RAM		EQU TK_MIN+1		* $DB
TK_PI			EQU TK_RAM+1		* $DC
TK_TWOPI		EQU TK_PI+1			* $DD
TK_VPTR		EQU TK_TWOPI+1		* $DE
TK_SADD		EQU TK_VPTR+1		* $DF
